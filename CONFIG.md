...
### ``armips\include\config.s`` and ``include\config.h``
 ...
 ``IMPLEMENT_HARDCODE_NUZLOCKE`` (``include/config.h`` only) enforces a nuzlocke-style capture rule in the engine itself: only the first non-dupe wild Pokémon encountered on a given map ("route") can be caught there, tracked with one bit per map in the save file. A shiny target is always catchable regardless of the route's lock state (shiny clause), and a species you've already caught doesn't lock the route when encountered (dupes clause). Applies to any non-trainer battle that isn't a Safari Zone, Pal Park, tutorial, Bug Catching Contest, or Totem encounter, which includes normal wild encounters, roamers, and scripted/static wild battles (e.g. gift Pokémon, legendaries). This requires ``ALLOW_SAVE_CHANGES`` to be defined, since the per-route lock state is stored as an expanded save field.
